...
export default {
    ...
    async execute(ctx) {
        ...
        await ctx.reply('ꕥ Procesando tu audio, por favor espera...');

        try {
            const result = await ytdlp('audio', ctx.args[0]);
            ...
            await ctx.reply('ꕤ Error al descargar el audio. El servicio puede estar caído o la URL es inválida.');
        }
    }
};
